...
class Reporter(models.Model):
    ...
    correct_registration = models.BooleanField(default=False, help_text = "Flag a correct CHW registration, for DB Clean ")
    ...
    def connection(self):
        """Returns the primary connection object of this Reporter. Ours, Rwanda, is from kannel_smpp backend"""

        try:
            if self.get_connections().exists():
                return self.get_connections().filter(backend__name = settings.PRIMARY_BACKEND)[0]
...
### Because it was found different logic were implemented to fetch
### registered reporters, deactivated, active and inactive irrespective of those correctly registered we have implemented
### the following functions to be used in all views trying to present statistics of CHWs


def get_wrong_registered_reporters():

    """ This function figures out any wrongly registered reporter in the CHW registry"""

    ## mistakenly registered chws'telephone_moh
    wrong_telephone_moh = Reporter.objects.extra(where=["CHAR_LENGTH(telephone_moh) != 13"])
    ## mistakenly registered chws'telephone_moh
    wrong_national_id = Reporter.objects.extra(where=["CHAR_LENGTH(national_id) != 16"])
    ##mistakenly registered chws'village
    wrong_village = Reporter.objects.filter(village = None)
    ##mistakenly registered chws'role
    wrong_role = Reporter.objects.filter(role = None)
    ## mistakenly registered chws'health_centre
    wrong_health_centre = Reporter.objects.filter(health_centre = None)

    ans = set()

    for w in wrong_telephone_moh:   ans.add(w)
    for w in wrong_national_id:   ans.add(w)
    for w in wrong_village:   ans.add(w)
    for w in wrong_role:   ans.add(w)
    for w in wrong_health_centre:   ans.add(w)

    return {'wrong': ans, 'wrong_telephone_moh': wrong_telephone_moh, 'wrong_national_id': wrong_national_id,\
             'wrong_village': wrong_village, 'wrong_role': wrong_role,'wrong_health_centre':wrong_health_centre}

def get_correct_registered_reporters():

    """ This function figures out correctly registered reporter in the CHW registry"""

    reporters = Reporter.objects.all()
    wrong = get_wrong_registered_reporters()['wrong']
    ## remove mistakenly registered chws 
    reporters = reporters.exclude(pk__in = [ rp.id for rp in wrong])

    return reporters

def get_deactivated_reporters():
    
    """ This function figures out deactivated correctly registered reporter in the CHW registry"""

    reporters = get_correct_registered_reporters()
    reporters = reporters.filter(deactivated = True)
...
def fix_reporters_numbers():
    rps =  Reporter.objects.extra(where=["CHAR_LENGTH(telephone_moh) != 13"])
    for r in rps:
        try:
            conns = Connection.objects.filter(contact__name = r.national_id).extra(where=["CHAR_LENGTH(identity) = 13"])
            r.telephone_moh = conns[0].identity
            r.save()
        except: continue
