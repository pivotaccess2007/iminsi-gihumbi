...
def export_model_as_csv(modeladmin, request, queryset):
    ...
    for obj in queryset:
        ...
        for field in field_list:
            ...
            else:   field_obj, attr, value = admin_util.lookup_field(field, obj, modeladmin)
            try:
                if field in has_name_fields:  csv_line_values.append(value.name)
                elif field in is_date_fields: csv_line_values.append("%d/%d/%d" % (value.day, value.month, value.year))
                else:   csv_line_values.append('%s' % value)
            except Exception, e:
                try:    csv_line_values.append('%s' % value)
                except: csv_line_values.append("NULL")
...
class CHWAdmin(admin.ModelAdmin):
    actions = (export_model_as_csv,export_model_as_excel)
    exportable_fields = ('surname', 'given_name', 'role', 'sex', 'education_level', 'date_of_birth', 'join_date', 'national_id', 'telephone_moh', 'village',\
                            'village__code', 'cell', 'cell__code', 'sector', 'sector__code', 'health_centre', 'health_centre__code', 'referral_hospital',
                            'referral_hospital__code', 'district', 'district__code', 'province', 'province__code')
