...
### WE NEED TO USE THE FOLLOWING VARIABLE TO DO OUR CHWS REPORTING RATE STATISTICS
TESTING_COUNTRY = Nation.objects.filter(name = 'TEST')
CORRECT_REGISTERED_CHWS = Reporter.objects.filter(correct_registration = True).exclude(nation__in = TESTING_COUNTRY)

def get_reporters_statistics():
    #from rapidsmsrw1000.apps.ubuzima.models import ReporterStatistic
    rps = []#ReporterStatistic.objects.all()
    return rps
REPORTER_STATISTICS = get_reporters_statistics()
...
@permission_required('chws.can_view')
def reporter_logs(request):
    request.base_template = "webapp/layout.html"
    from rapidsmsrw1000.apps.messagelog.tables import MessageTable
    reports = messages = []
    filters = {'period':default_period(request)}
    start_date = filters['period']['start']    
    end_date = filters['period']['end'] + datetime.timedelta(days = 1)
        
    if request.REQUEST.has_key("reporter") : 
        try:
            reporter = Reporter.objects.filter(pk = int(request.GET['reporter']))
            if reporter.exists():
                reporter = reporter[0]
                reports = reporter.reportreporter.filter( created__gte = start_date, created__lte = end_date).order_by('-created')
                connection = Connection.objects.filter(identity = reporter.telephone_moh)#;print connection
                messages = Message.objects.filter( connection = connection, date__gte = start_date, date__lte = end_date ).order_by('-date')#; print messages.count()
        except:
            pass

    #print results
    return render_to_response("chws/searchreporter.html", dict(reports = paginated(request, reports), messages = paginated(request, messages),user=request.user, \
                                                               start_date = date.strftime(start_date, '%d.%m.%Y'),
                                                               end_date = date.strftime(end_date, '%d.%m.%Y')), context_instance=RequestContext(request)) 
...
def chwreg(request):
    ...
    try:        
        ...
        if request.REQUEST.has_key('reporter'):
            ...
            if vlls is None:    vlls = Village.objects.filter(cell = results.cell)
            if clls is None:    clls = Cell.objects.filter(sector = results.sector)
            if scs is None: scs = Sector.objects.filter(district = results.district)
            if hcs is None:  hcs = HealthCentre.objects.filter(district = results.district)
            if hps is None: hps = Hospital.objects.filter(district = results.district)
            if dsts is None:    dsts = District.objects.filter(province = results.province)
            if prvs is None: prvs = Province.objects.filter(nation = results.nation)
        ...
        if request.REQUEST.has_key('activated'):
            ...
            try:
                from rapidsmsrw1000.apps.ubuzima.models import ReporterStatistic
                rpst = ReporterStatistic.objects.get(reporter = results, date = datetime.date.today())
                rpst.deactivated = results.deactivated
                rpst.save()
            except Exception, e:
                pass
            results.save()
        ...
        if request.REQUEST.has_key("nid") or request.REQUEST.has_key("telephone_moh"): 
            ...
            try:
                ...
                reporter.date_of_birth   =	get_date(request.POST['dob'].replace(".", '/'))
                reporter.join_date		=   get_date(request.POST['jod'].replace(".", '/'))
...
def group_messages(request):
    """Messaging Feature."""
    request.base_template = "webapp/layout.html"
    if get_user_location(request).user.username not in ["mikemi", "mugitera@gmail.com"]:#!= 'mikemi':#'gaju':        
        return render_to_response("ubuzima/404.html",{'error':"YOU ARE NOT ALLOWED TO SEND GROUP MESSAGES. PLEASE CONTACT Mike Misengo and Baptiste Byiringiro"}, context_instance=RequestContext(request))
    hc = dst = None
    sent = []
...
def inactive_reporters(req, rez, group):
    reps = REPORTER_STATISTICS.filter(date = datetime.date.today(), active = False, deactivated = False)
    reps = reps.filter(reporter__role=group, **rez)
    pst = reporter_fresher(req)
    return reps.filter(**pst)


def active_reporters(req, rez, group):
    reps = REPORTER_STATISTICS.filter(date = datetime.date.today(), active = True, deactivated = False)
    reps = reps.filter(reporter__role=group, **rez)
    pst = reporter_fresher(req)
    return reps.filter(**pst)
...
def activity_statistics(req):
    ...
    #active_reporters = REPORTER_STATISTICS.filter(date = end, active = True, deactivated = False).filter(**rez).filter(**pst)
    #inactive_reporters = REPORTER_STATISTICS.filter(date = end, active = False, deactivated = False).filter(**rez).filter(**pst)
...
def chws_attendance_daily(req, rez, pst, days_b):
    ...
    for day in days_b:
        ...
        asm = REPORTER_STATISTICS.filter(reporter__role__code = 'asm',date = end,  deactivated = False).filter(**rez).filter(**pst)
        binome = REPORTER_STATISTICS.filter(reporter__role__code = 'binome',date = end,  deactivated = False).filter(**rez).filter(**pst)
        
        asm_total = asm.count()
        binome_total = binome.count() 
          
        active_asm = active_reporters.filter(reporter__role__code = 'asm')
        inactive_asm = inactive_reporters.filter(reporter__role__code = 'asm')
        active_binome = active_reporters.filter(reporter__role__code = 'binome')
        inactive_binome = inactive_reporters.filter(reporter__role__code = 'binome')
        reporters[d]['active_asm'] = active_asm
        reporters[d]['inactive_asm'] = inactive_asm
        reporters[d]['active_binome'] = active_binome
        reporters[d]['inactive_binome'] = inactive_binome
        
        active_asm_total = active_asm.count()
        inactive_asm_total = inactive_asm.count()
        active_binome_total = active_binome.count()
        inactive_binome_total = inactive_binome.count()
        
        if asm_total == 0:
            total['active_asm'][d] =  0
            total['inactive_asm'][d] = 0
            reporters[d]['active_asm_percent'] = float(0)
            reporters[d]['inactive_asm_percent'] = float(0)
        else:
            total['active_asm'][d] =  float(active_asm_total)*100/asm_total
            total['inactive_asm'][d] = float(inactive_asm_total)*100/asm_total
            reporters[d]['active_asm_percent'] = float(active_asm_total)*100/asm_total
            reporters[d]['inactive_asm_percent'] = float(inactive_asm_total)*100/asm_total
                        
        if binome_total == 0:
            total['active_binome'][d] = 0
            total['inactive_binome'][d] = 0
            reporters[d]['active_binome_percent'] = float(0)
            reporters[d]['inactive_binome_percent'] = float(0)
        else:
            total['active_binome'][d] = float(active_binome_total)*100/binome_total
            total['inactive_binome'][d] = float(inactive_binome_total)*100/binome_total
            reporters[d]['active_binome_percent'] = float(active_binome_total)*100/binome_total
            reporters[d]['inactive_binome_percent'] = float(inactive_binome_total)*100/binome_total
